	...
	namespace Functions
	{
		namespace Visuals
		{
			...
			void ESP::PlayerBox ( const int &Index, const int &x, const int &w, const int &PlayerBoxHeight, const Vector2D &ScreenTop )
			{
				...
				{
					Renderer::g_Drawing->DrawCoalBox(int(ScreenTop[0] - (PlayerBoxHeight* 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
						int(PlayerBoxHeight), BoxColor[0], BoxColor[1], BoxColor[2], 255);
				}
